...
const buildAdminMetrics = () =>
  Array.from({ length: 30 }).map((_, i) => ({
    day: `${i + 1}`,
    newUsers: Math.round(Math.random() * 30),
    dau: Math.round(Math.random() * 100),
  }));
const buildRevenueChannels = () =>
  Array.from({ length: 7 }).map((_, i) => ({
    day: `D${i + 1}`,
    direct: Math.round(Math.random() * 1000),
    subs: Math.round(Math.random() * 600),
    wholesale: Math.round(Math.random() * 400),
  }));
    ...
    <div>
      ...
      <div className="mt-6 rounded-2xl bg-white p-4 shadow">
        ...
        <p className="mt-2 text-sm text-[#6e4b33]">
          Quick list of seeded users and role toggles.
        </p>
      </div>
    </div>
  );
