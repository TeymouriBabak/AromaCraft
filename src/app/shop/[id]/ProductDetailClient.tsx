...
import { type Product, reviews } from "@/data/products-multi-brand";
...
const GRIND_OPTIONS = ["Whole Bean", "Cafetiere", "Filter", "Espresso"] as const;
...
export default function ProductDetailClient({ product }: ProductDetailClientProps) {
  ...
  const [selectedGrind, setSelectedGrind] = useState<string>(product.grindTypes[0] ?? "Whole Bean");
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 lg:px-8">
      <div className="mb-8 flex items-center gap-4 text-sm text-[#6e4b33]">
        <Link href="/shop" className="inline-flex items-center gap-2 rounded-full border border-[#d4a373]/20 bg-[#f9f6f0] px-4 py-2 text-[#1a0f0a] transition hover:bg-[#efe2d2]">
          <ChevronLeft size={16} /> Back to shop
        </Link>
        <span className="rounded-full bg-[#f1e5d1] px-3 py-1">{product.brand}</span>
      </div>
      ...
      <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="rounded-3xl border border-[#d4a373]/20 bg-[#efe2d2] p-8 text-center">
            <div className="mx-auto flex h-72 w-48 items-center justify-center rounded-3xl border border-[#8c5e3d] bg-[#1a0f0a] text-[#f6e5d1] shadow-2xl">
              <div className="text-center">
                <p className="text-sm uppercase tracking-[0.3em]">{product.country}</p>
        ...
        <div>
          <p className="text-sm uppercase tracking-[0.35em] text-[#d4a373]">{product.badge}</p>
          <h1 className="mt-3 font-serif text-4xl text-[#1a0f0a] dark:text-[#f6e5d1]">{product.name}</h1>
          <p className="mt-3 text-lg text-[#6e4b33] dark:text-[#e8d8c0]">{product.description}</p>
          ...
          <div className="mt-6 rounded-3xl border border-[#d4a373]/20 bg-white/70 p-5 dark:bg-[#23110c]">
            <div className="flex items-baseline justify-between">
              <p className="text-sm uppercase tracking-[0.3em] text-[#d4a373]">Price</p>
              <p className="font-serif text-3xl text-[#1a0f0a] dark:text-[#f6e5d1]">${product.price}</p>
            </div>
            ...
            <button
              ...
              onClick={() => {
                addItem({ productId: product.id, quantity: 1, name: product.name, price: product.price, image: product.image, grindType: selectedGrind, size: selectedSize });
                openCart();
              }}
            >
      ...
      <div className="mt-12 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
        <div className="rounded-4xl border border-[#d4a373]/20 bg-white/70 p-6 shadow-sm dark:bg-[#23110c]">
          ...
          <ol className="mt-4 space-y-3 text-sm text-[#6e4b33] dark:text-[#e8d8c0]">
            ...
            <li>3. Finish around 2:30–3:00 minutes for clarity and sweetness.</li>
          </ol>
        </div>

        <div className="rounded-4xl border border-[#d4a373]/20 bg-[#f9f6f0] p-6 shadow-sm dark:bg-[#23110c]">
          <h2 className="font-serif text-2xl text-[#1a0f0a] dark:text-[#f6e5d1]">Reviews</h2>
          <div className="mt-5 space-y-4">
            {reviews.map((review) => (
              <div key={review.id} className="rounded-[1.25rem] border border-[#d4a373]/20 bg-white/80 p-4 dark:bg-[#1a0f0a]">
                <div className="flex items-center justify-between">
                ...
                <p className="mt-3 text-sm text-[#6e4b33] dark:text-[#e8d8c0]">{review.content}</p>
              </div>
            ))}
          </div>
        </div>
