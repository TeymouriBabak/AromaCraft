  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      {items.map((item) => (
        <div
          key={item.id}
          className="rounded-[1.5rem] border border-[#d4a373]/20 bg-white/80 p-4 shadow-sm dark:bg-[#23110c]"
        >
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              <p className="text-xs uppercase tracking-[0.22em] text-[#b56e3b]">
                {normalizeCommentDestination(item.destination) === 'pike-place'
                  ? 'Pike Place'
                  : 'Home'}
              </p>
              <p className="mt-1 font-semibold text-[#1a0f0a] dark:text-[#f6e5d1]">
                {item.author}
              </p>
            </div>
            <StarRow rating={item.rating} />
          </div>
          <p className="text-sm leading-6 text-[#6e4b33] dark:text-[#e8d8c0]">
            {item.content}
          </p>
        </div>
      ))}
...
export default function ReviewsPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/reviews')
      ...
      .then((payload) => {
        if (!active) return;
        const nextReviews = Array.isArray(payload?.data?.reviews)
  ...
  const homeReviews = useMemo(
    () =>
      reviews.filter(
        (item) => normalizeCommentDestination(item.destination) === 'home'
      ),
    [reviews]
  );
  const pikePlaceReviews = useMemo(
    () =>
      reviews.filter(
        (item) => normalizeCommentDestination(item.destination) === 'pike-place'
      ),
    [reviews]
  );
  ...
  const handleSubmit = async () => {
    if (!isAuthenticated) {
      setStatus({
        type: 'error',
        text: 'Please sign in to share your feedback.',
      });
      return;
    }
    ...
    if (content.trim().length < 12) {
      setStatus({
        type: 'error',
        text: 'Please share a few more details about your experience.',
      });
      return;
    }
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="mb-8 rounded-4xl border border-[#d4a373]/20 bg-white/80 p-5 shadow-sm dark:bg-[#1a0f0a]">
        ...
        <textarea
          ...
          placeholder={
            isAuthenticated
              ? 'Tell us how your experience felt...'
        ...
        {status ? (
          <div
            className={`mt-4 rounded-2xl border px-4 py-3 text-sm ${status.type === 'success' ? 'border-[#2f7d4a]/20 bg-[#f3fbf6] text-[#2f7d4a]' : 'border-[#e76f51]/20 bg-[#fff7f5] text-[#b24c33]'}`}
          >
            {status.text}
          </div>
        ) : null}
      ...
      <div className="space-y-8">
        <section className="rounded-4xl border border-[#d4a373]/20 bg-white/80 p-5 shadow-sm dark:bg-[#1a0f0a]">
          <div className="mb-5 flex items-center justify-between">
            ...
            <span className="rounded-full border border-[#d4a373]/25 bg-[#f9f6f0] px-3 py-1 text-xs uppercase tracking-[0.2em] text-[#6e4b33] dark:bg-[#23110c] dark:text-[#e8d8c0]">
              Global feedback
            </span>
          </div>
          <ReviewList
            items={
              homeReviews.length
                ? homeReviews
                : defaultReviews.filter(
                    (item) =>
                      normalizeCommentDestination(item.destination) === 'home'
                  )
            }
          />
        </section>
        ...
        <section className="rounded-4xl border border-[#d4a373]/20 bg-white/80 p-5 shadow-sm dark:bg-[#1a0f0a]">
          ...
          <ReviewList
            items={
              pikePlaceReviews.length
                ? pikePlaceReviews
                : defaultReviews.filter(
                    (item) =>
                      normalizeCommentDestination(item.destination) ===
                      'pike-place'
                  )
            }
          />
