...
export default function SiteShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-(--page-bg) text-(--text-primary) transition-colors duration-300">
      <div
        className="border-b border-[#3d2d24]/20 bg-[#1a0f0a] px-3 py-2 text-center text-[11px] uppercase tracking-[0.3em] text-[#f6e5d1]"
        aria-live="polite"
      >
      ...
      <header
        className={`sticky top-0 z-50 border-b border-black/10 bg-[#f9f6f0]/80 backdrop-blur-xl transition-all duration-300 dark:border-white/10 dark:bg-[#1a0f0a]/90 ${isScrolled ? 'shadow-[0_10px_40px_-24px_rgba(43,29,23,0.35)]' : 'shadow-none'}`}
      >
        <div
          className={`mx-auto flex max-w-7xl items-center justify-between px-4 transition-all duration-300 lg:px-8 ${isScrolled ? 'py-3' : 'py-4'}`}
        >
          ...
          <nav className="hidden items-center gap-7 lg:flex">
            {navItems.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className="text-sm font-medium text-[#3d2d24] transition hover:text-[#d4a373] dark:text-[#f9f6f0]"
              >
                {item.label}
              </Link>
            ))}
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <button
              onClick={toggleTheme}
              className="rounded-full border border-[#d4a373]/40 bg-white/70 p-2.5 transition hover:-translate-y-0.5"
            ...
            <button
              ...
              className="rounded-full border border-[#d4a373]/40 bg-white/70 p-2.5 lg:hidden"
                    ...
                    {profileOpen ? (
                      ...
                      >
                        <div className="flex items-center gap-3 rounded-xl bg-[#f9f6f0] p-3 dark:bg-[#2b1a13]">
                          ...
                          <div>
                            <p className="font-semibold text-[#1a0f0a] dark:text-[#f6e5d1]">
                              {user?.username}
                            </p>
                        ...
                        <div className="mt-3 space-y-2 text-sm">
                          <Link
                            href="/dashboard"
                            className="flex items-center gap-2 rounded-xl px-3 py-2 text-[#3d2d24] transition hover:bg-[#f9f6f0] dark:text-[#f6e5d1] dark:hover:bg-[#2b1a13]"
                          >
              ...
              <div className="flex flex-col gap-2">
                {navItems.map((item) => (
                  <Link
                    key={item.label}
                    href={item.href}
                    className="rounded-full px-3 py-2 text-sm font-medium text-[#3d2d24] transition hover:bg-[#f6e8dc] dark:text-[#f9f6f0] dark:hover:bg-[#2f1f16]"
          ...
          {query && (
            ...
            >
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-[#7a5b45] dark:text-[#f6e5d1]">
                  Instant matches
                </p>
                <button
                  onClick={() => setQuery('')}
                  className="text-sm text-[#e76f51]"
                >
                  Clear
                </button>
              </div>
              <div className="mt-3 grid gap-2 md:grid-cols-2">
                {filteredProducts.slice(0, 4).map((product) => (
                  <Link
                    ...
                    className="flex items-center justify-between rounded-xl border border-[#f0e3d5] bg-[#f9f6f0] p-3 dark:border-[#4d3126] dark:bg-[#22110c]"
                  >
                    <div>
        ...
        {isOpen && (
          ...
          >
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={() => closeCart()}
                className="rounded-full border border-[#d4a373]/40 p-2"
              >
                ×
              </button>
            </div>
            ...
            <div className="mt-8 rounded-2xl border border-[#d4a373]/20 bg-white/70 p-4 dark:bg-[#29130d]">
              ...
              <p className="mt-2 text-sm text-[#7a5b45]">
                {subtotal >= 50
                  ? 'Free shipping unlocked'
                  : `$${(50 - subtotal).toFixed(0)} away from free shipping`}
              </p>
            </div>
            ...
            <div className="mt-6 flex-1 rounded-2xl border border-[#d4a373]/20 bg-white/70 p-4 dark:bg-[#29130d]">
              {items.length === 0 ? (
                <p className="text-sm text-[#7a5b45]">
                  Your cart is empty. Add a roast to get started.
                </p>
              ) : (
                items.map((item) => (
                  <div
                    key={item.productId}
                    className="mb-3 rounded-xl bg-[#1a0f0a] p-3 text-[#f9f6f0]"
                  >
                    <div className="flex items-start justify-between gap-4">
                      <div>
                        <p className="font-semibold">{item.name}</p>
                        <p className="text-sm opacity-80">
                          {item.size ?? '250g'} · ${item.price.toFixed(2)} each
                        </p>
            ...
            <Link
              href="/checkout"
              onClick={() => closeCart()}
              className="mt-6 flex items-center justify-center rounded-full bg-[#e76f51] px-4 py-3 font-semibold text-white"
            >
      ...
      <footer className="border-t border-black/10 bg-[#1a0f0a] text-[#f6e5d1]">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-12 lg:grid-cols-4 lg:px-8">
          <section aria-labelledby="footer-branding">
            <p id="footer-branding" className="font-serif text-2xl">
              AromaCraft
            </p>
            <p className="mt-4 text-sm leading-7 text-[#d9c2a9]">
              Specialty coffee with traceable sourcing, slow roasting, and a
              luxury ritual for your daily cup.
            </p>
          </section>
          <nav aria-labelledby="footer-shop" className="space-y-3">
            <p
              id="footer-shop"
              className="font-semibold uppercase tracking-[0.3em] text-[#d4a373]"
            >
              Shop
            </p>
            <ul className="mt-4 space-y-2 text-sm text-[#e8d8c0]">
              <li>
                <Link href="/shop">Single Origin</Link>
              </li>
              <li>
                <Link href="/shop">Blends</Link>
              </li>
              <li>
                <Link href="/shop">Espresso</Link>
              </li>
              <li>
                <Link href="/shop">Subscriptions</Link>
              </li>
            </ul>
          </nav>
          <nav aria-labelledby="footer-support" className="space-y-3">
            ...
            <ul className="mt-4 space-y-2 text-sm text-[#e8d8c0]">
              ...
              <li>
                <a href="mailto:hello@aromacraft.com">hello@aromacraft.com</a>
              </li>
            </ul>
          </nav>
          <section
          ...
          >
            ...
            <p className="text-sm text-[#d9c2a9]">
              Private offers and new release drops.
            </p>
          </section>
        </div>
        <div className="border-t border-white/10 px-4 py-4 text-sm text-[#d9c2a9] lg:px-8">
          <div className="mx-auto flex max-w-7xl flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap items-center gap-4">
              <p className="text-sm uppercase tracking-[0.24em] text-[#d4a373]">
                Follow the ritual
              </p>
              <div
