    ...
    <aside className="sticky top-20 rounded-4xl border border-[#d4a373]/20 bg-[#f9f6f0] p-6 shadow-sm dark:bg-[#23110c]">
      ...
      <div className="mt-4 space-y-3 text-sm text-[#6e4b33] dark:text-[#e8d8c0]">
        ...
        <div className="border-t pt-3">
          ...
          <div className="mt-2 flex justify-between font-semibold text-[#1a0f0a] dark:text-[#f6e5d1]">
            <span>Total</span>
            <span>${total.toFixed(2)}</span>
          </div>
        </div>
      </div>
      <p className="mt-4 text-sm text-[#7a5b45] dark:text-[#d9c2a4]">
        {items.length
          ? `${items.length} item${items.length === 1 ? '' : 's'} in your cart.`
          : 'Add products from the shop to proceed.'}
      </p>
    </aside>
  );
}
