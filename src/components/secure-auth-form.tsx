...
export default function SecureAuthForm({
  onAuthenticated,
  ...
  const [firstNameStatus, setFirstNameStatus] =
    useState<AvailabilityState>('idle');
  const [lastNameStatus, setLastNameStatus] =
    useState<AvailabilityState>('idle');
  ...
  const loginMode = useWatch({
    control: loginControl,
    name: 'loginMode',
    defaultValue: defaultLoginValues.loginMode,
  });
  const loginRole = useWatch({
    control: loginControl,
    name: 'role',
    defaultValue: defaultLoginValues.role,
  });
  const signupValues = useWatch({
    control: signupControl,
    defaultValue: defaultSignupValues,
  }) as SignupFormValues;
  const signupPassword = signupValues.password ?? defaultSignupValues.password;
  const signupConfirmPassword =
    signupValues.confirmPassword ?? defaultSignupValues.confirmPassword;
    ...
    const timeout = window.setTimeout(() => {
        ...
        (async (value) => {
          try {
            ...
            if ((signupValues.username ?? '').trim() === value) {
              setUsernameStatus(
                json?.ok && json.data && json.data.available
                  ? 'available'
                  : 'taken'
              );
            }
      ...
      if (!mobileValue) {
        ...
        (async (value) => {
          try {
            const resp = await fetch(
              `/api/auth/check-mobile?mobile=${encodeURIComponent(value)}`
            );
            const json = await resp.json();
  ...
  const onSignup = async (values: SignupFormValues) => {
    if (!isSignupReady) {
      setMessage({
        type: 'error',
        text: 'Please complete every required field before creating your account.',
      });
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setMessage({
          type: 'success',
          text: 'We sent a verification code to your email. Enter it to activate your account.',
        });
        return;
      }
      ...
      <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr]">
        ...
        >
          ...
          <p className="mt-4 max-w-2xl text-lg leading-8 text-[#6e4b33] dark:text-[#e8d8c0]">
            Sign in or create an account to save favorites, track orders, and
            experience a more personal coffee ritual.
          </p>
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            {[
              ...
              <div
                key={item}
                className="rounded-2xl border border-[#d4a373]/20 bg-white/70 p-4 text-sm text-[#5f473d] dark:bg-[#29130d]"
              >
                {item}
              </div>
            ))}
          ...
          <AnimatePresence mode="wait">
            {view === 'login' && loginStep === 'form' && verificationStep === 'form' ? (
              ...
              >
                  ...
                  {loginModeOptions.map((mode) => (
                    ...
                    >
                      <div className="flex items-center justify-between gap-2">
                        ...
                        {loginMode === mode.id ? (
                          <CheckCircle2 size={16} className="text-[#2f7d4a]" />
                        ) : null}
                      </div>
                ...
                >
                  <div>
                    <label className="mb-2 block text-sm font-semibold text-[#3d2d24] dark:text-[#f6e5d1]">
                      Login as
                    </label>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <label className="flex items-center gap-2 text-[#6e4b33] dark:text-[#e8d8c0]">
                      <input
                        ...
                        className="rounded border-[#d4a373]"
                      />
                      Remember me
                    </label>
                    <div className="flex gap-3 text-[#b56e3b]">
                      <button
                        ...
                        className="font-medium hover:text-[#e76f51]"
                      >
                        Forgot Username?
                      </button>
                      <button
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center rounded-[1.25rem] border border-[#d4a373]/25 bg-[#f9f6f0] px-3 py-3 dark:bg-[#23110c]">
                      ...
                      />
                      {firstNameStatus === 'available' ? (
                        <CheckCircle2 size={16} className="text-[#2f7d4a]" />
                      ) : null}
                      {firstNameStatus === 'taken' ? (
                        <X size={16} className="text-[#e76f51]" />
                      ) : null}
                    </div>
                    ...
                    {firstNameStatus === 'taken' ? (
                      <p className="mt-2 text-sm text-[#e76f51]">
                        That first name is already registered.
                      </p>
                    ) : null}
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center rounded-[1.25rem] border border-[#d4a373]/25 bg-[#f9f6f0] px-3 py-3 dark:bg-[#23110c]">
                      ...
                      />
                      {lastNameStatus === 'available' ? (
                        <CheckCircle2 size={16} className="text-[#2f7d4a]" />
                      ) : null}
                      {lastNameStatus === 'taken' ? (
                        <X size={16} className="text-[#e76f51]" />
                      ) : null}
                    </div>
                    ...
                    {lastNameStatus === 'taken' ? (
                      <p className="mt-2 text-sm text-[#e76f51]">
                        That last name is already registered.
                      </p>
                    ) : null}
                  </div>
                  ...
                  <div>
                    ...
                    {usernameStatus === 'available' ? (
                      <p className="mt-2 text-sm text-[#2f7d4a]">
                    ...
                    usernameStatus !== 'checking' ? (
                      <ul className="mt-2 space-y-1 text-sm text-[#e76f51]">
                        {usernameRequirements.map((rule) => (
                          ...
                          >
                            {rule.valid ? (
                              <Check size={14} className="text-[#2f7d4a]" />
                            ) : (
                  ...
                  <div>
                      ...
                      render={({ field }) => (
                        <div className="rounded-[1.25rem] border border-[#d4a373]/25 bg-[#f9f6f0] p-3 dark:bg-[#23110c]">
                          <PhoneInput
                            ...
                            value={field.value ?? ''}
                            onChange={(value) =>
                              field.onChange(normalizePhoneNumber(value ?? ''))
                            }
                            className="w-full"
                            numberInputProps={{
                              className:
                    ...
                    {mobileStatus === 'available' ? (
                      <p className="mt-2 flex items-center gap-2 text-sm text-[#2f7d4a]">
                        <CheckCircle2 size={14} /> Mobile number is available.
                      </p>
                    ) : null}
                    {mobileStatus === 'taken' ? (
                      <p className="mt-2 text-sm text-[#e76f51]">
                    ...
                    {signupPassword ? (
                      <div className="mt-3">
                        ...
                        <div className="h-2 rounded-full bg-[#efe2d2]">
                          <div
                            className={`h-2 rounded-full transition-all duration-300 ${passwordStrength.bar}`}
  ...
  void uploadAvatar(file);
                    ...
                    >
                      ...
                      <div>
                        ...
                        <p className="text-sm text-[#6e4b33] dark:text-[#e8d8c0]">
                          Drag and drop or browse for a profile image
                        </p>
                      </div>
                    </motion.label>
                    <input
                    ...
                    {avatarPreview ? (
                      <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        className="mt-3 flex items-center gap-3 rounded-[1.25rem] border border-[#d4a373]/20 bg-[#f9f6f0] p-3 dark:bg-[#23110c]"
                      >
              ...
              >
                <div className="rounded-[1.25rem] border border-[#d4a373]/20 bg-[#f9f6f0] p-4 dark:bg-[#23110c]">
                  ...
                  <p className="mt-2 text-sm text-[#6e4b33] dark:text-[#e8d8c0]">
                    We sent a verification email to {pendingEmail}. Check your
                    inbox and enter the 6-digit code, or click the link in the
                    email.
                  </p>
                </div>
                <div>
