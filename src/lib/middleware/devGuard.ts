 ...
 */
export function devOnly(handler: (req: NextApiRequest, res: NextApiResponse) => Promise<void> | void) {
  return async (req: NextApiRequest, res: NextApiResponse) => {
    // In production or non-development environments, always reject
    if (process.env.NODE_ENV !== 'development') {
      return sendNotFound(res, 'This endpoint is not available in this environment.');
    }
    ...
    try {
      ...
      console.error('[devGuard] Unexpected error in development endpoint:', error instanceof Error ? error.message : 'Unknown error');
