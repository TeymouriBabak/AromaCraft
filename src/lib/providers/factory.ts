...
export function getEmailProvider(name?: EmailProviderName) {
  const explicitProvider =
    name || (process.env.EMAIL_PROVIDER as EmailProviderName | undefined);
  if (process.env.NODE_ENV === 'production' && !explicitProvider) {
    throw new Error(
      'EMAIL_PROVIDER must be configured in production. Mock providers are disabled.'
    );
  }
...
export function getSmsProvider(name?: SmsProviderName) {
  const explicitProvider =
    name || (process.env.SMS_PROVIDER as SmsProviderName | undefined);
  if (process.env.NODE_ENV === 'production' && !explicitProvider) {
    throw new Error(
      'SMS_PROVIDER must be configured in production. Mock providers are disabled.'
    );
  }
...
export function getStorageProvider(name?: StorageProviderName) {
  const explicitProvider =
    name || (process.env.STORAGE_PROVIDER as StorageProviderName | undefined);
  if (process.env.NODE_ENV === 'production' && !explicitProvider) {
