...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    return jsonSuccess(res, { overview: { totalUsers, totalCustomers, totalAdmins, newUsersThisMonth } }, 200);
  } catch (error) {
    void error;
    return jsonError(res, 'server_error', 'Unable to fetch manager overview', 500);
  }
}
