import { getEmailProvider, getSmsProvider, getStorageProvider } from '../src/lib/providers/factory'
...
function ensureNoSecretPayload(payload: unknown, endpoint: string, allowDevSecret = false) {
  if (allowDevSecret) {
    return
  }

  // OTP-related fields that should never appear in normal responses
  const otpFields = ['otp', 'otpCode', 'verificationCode', 'verificationToken', 'otpHash', 'code'];
  // Sensitive fields
  const sensitiveFields = ['passwordHash', 'password', 'token', 'session', 'secret', 'authorization', 'cookie', 'sessionToken'];
  ...
  for (const path of allPaths) {
    ...
    const isInDataSection = path.startsWith('data.') || path.startsWith('user.') || path.startsWith('meta.');
    
    if (otpFields.includes(lastKey) && isInDataSection) {
      throw new Error(`OTP field ${lastKey} (path: ${path}) exposed in ${endpoint}`)
    }
    
    if (sensitiveFields.includes(lastKey)) {
      throw new Error(`Sensitive field ${lastKey} (path: ${path}) exposed in ${endpoint}`)
    }
  }
}

function logResponseStructure(res: { status: number; json: unknown }, endpoint: string) {
  const keys = getSafeKeyPaths(res.json);
  console.log(`RESPONSE_KEYS[${endpoint}]`, `status=${res.status}`, `keys=[${keys.slice(0, 10).join(', ')}${keys.length > 10 ? '...' : ''}]`);
}
...
async function run() {
  ...
  const authProviders = {
    ...
    resolved: {
      email: getEmailProvider() === mailpitProvider ? 'local_mailpit' : 'unknown',
      sms: getSmsProvider() === mockSmsProvider ? 'mock_sms' : 'unknown',
      storage: getStorageProvider() === localStorageProvider ? 'local_files' : 'unknown',
    },
  }
  ...
  if (!signup.ok) {
    ...
    throw new Error(`Signup failed: ${signup.json?.error?.message || 'unknown error'}`)
  }
  ...
  const latest = await post('/api/dev/latest-verification-code', { email: EMAIL })
  ...
  console.log('ME', me.status, me.json?.authenticated ? 'authenticated' : 'unauthenticated', me.json?.user?.emailVerified ? 'verified' : 'unverified')
  ...
  console.log('REUSE', reuse.status, reuse.ok ? 'unexpected_allowed' : 'rejected_correct')
  ...
  const latest2 = await post('/api/dev/latest-verification-code', { email: EMAIL })
  logResponseStructure(latest2, '/api/dev/latest-verification-code (second)')
  ensureNoSecretPayload(latest2.json, '/api/dev/latest-verification-code (second)', true)
  ...
  const oldAfterRotate = await post('/api/auth/verify-account', { email: EMAIL, code })
  logResponseStructure(oldAfterRotate, '/api/auth/verify-account (old after rotate)')
  ensureNoSecretPayload(oldAfterRotate.json, '/api/auth/verify-account (old after rotate)')
  console.log('REUSE_OLD_AFTER_ROTATE', oldAfterRotate.status, oldAfterRotate.ok ? 'unexpected_allowed' : 'rejected_correct')
  ...
  console.log('RATE1', rate1.status, rate1.ok ? 'allowed' : rate1.json?.error?.code || 'blocked')
  console.log('RATE2', rate2.status, rate2.ok ? 'allowed' : rate2.json?.error?.code || 'blocked')
  console.log('RATE3', rate3.status, rate3.ok ? 'allowed' : rate3.json?.error?.code || 'blocked')

  const rate4 = await post('/api/auth/resend-verification', { email: EMAIL })
  console.log('RATE4', rate4.status, rate4.ok ? 'allowed' : rate4.json?.error?.code || 'blocked')
...
run().catch((err) => {
  console.error('SMOKE_TEST_FAILED', err instanceof Error ? err.message : String(err))
  process.exit(1)
})
