...
import { handleLogin, handleVerifyAccount, handleResendVerification } from '../src/lib/auth-utils';
import { createDbUser, createDbSession } from '../src/lib/db-auth';
import { prisma } from '../src/lib/prisma';

async function clearTestState() {
  try {
    if ((globalThis as any).__aromacraftMockVerificationStore) (globalThis as any).__aromacraftMockVerificationStore.clear();
    if ((globalThis as any).__aromacraftMockOtpByEmail) (globalThis as any).__aromacraftMockOtpByEmail.clear();
  } catch {
    // ignore
  }
  try {
...
async function runForgotPassword() {
  ...
  try {
    ...
    assert.equal(consoleText.includes(email), false, `Captured console contained email: ${consoleText}`);
    assert.equal(consoleText.toLowerCase().includes('token'), false, `Captured console contained token substring: ${consoleText}`);
    assert.equal(/reset[-_ ]?link|\/reset|\?.*token=|token=|expires|expiry/i.test(consoleText), false, `Captured console contained reset link-like text: ${consoleText}`);
...
async function runLoginRate() {
  ...
  for (let attempt = 1; attempt <= 6; attempt += 1) {
    const res = makeMockRes();
    await (handleLogin as any)({ method: 'POST', headers: { 'x-forwarded-for': ip }, body: { identifier: process.env.TEST_CUSTOMER_EMAIL || 'tbabak@example.com', password: process.env.TEST_CUSTOMER_PASSWORD || 'Test-Password-123!', role: 'customer' } }, res);
    const out = res._get();
...
async function runSignupRate() {
  ...
  if (typeof handleSignupFn !== 'function') {
    console.error('handleSignup not found'); process.exitCode = 2; return;
  }
  for (let attempt = 1; attempt <= 6; attempt += 1) {
    const res = makeMockRes();
    await handleSignupFn({ method: 'POST', headers: { 'x-forwarded-for': '203.0.113.99' }, body: { firstName: 'Signup', lastName: 'Tester', gender: 'Female', username: `${username}_${attempt}`, mobile: `+1415555${String(7000 + attempt).padStart(4, '0')}`, email, password: 'SignupPass!23', confirmPassword: 'SignupPass!23' } }, res);
    const out = res._get();
    lastStatus = out.statusCode;
    lastBody = out.body as Record<string, unknown> | null;
  }
...
async function runOtpResend() {
  ...
  const user = await createDbUser({ username: `Otp${Date.now()}`, email, password: 'OtpPass!23', role: 'customer', firstName: 'Opt', lastName: 'Tester', gender: 'Female', mobile: `+1415555${String(2000 + (Date.now() % 9000)).padStart(4, '0')}`, countryCode: '+1' });
  if (!user) { console.error('setup user failed'); process.exitCode = 2; return; }
  const firstCode = '123456';
  const firstToken = randomUUID();
  const { hashOtp } = await import('../src/lib/auth/otp');
  const hashed = await hashOtp(firstCode);
  await prisma.verificationToken.create({ data: { id: randomUUID(), userId: user.id, type: 'EMAIL_VERIFICATION', token: firstToken, otpHash: hashed, expiresAt: new Date(Date.now() + 10 * 60 * 1000), createdAt: new Date(Date.now() - 5_000), verified: false, usedCount: 0, attemptCount: 0 } });

  const authMod = await import('../src/lib/auth-utils');
  const handleResendFn = authMod.handleResendVerification ?? authMod.default?.handleResendVerification;
  const getLatestMockVerificationOtp = authMod.getLatestMockVerificationOtp ?? authMod.default?.getLatestMockVerificationOtp;
  const handleVerifyFn = authMod.handleVerifyAccount ?? authMod.default?.handleVerifyAccount;
  if (typeof handleResendFn !== 'function' || typeof getLatestMockVerificationOtp !== 'function' || typeof handleVerifyFn !== 'function') {
    console.error('auth utils functions not available'); process.exitCode = 2; return;
  }
  ...
  try {
    if (!latestCode || latestCode === firstCode) throw new Error('latestCode invalid');
    const staleRow = await prisma.verificationToken.findFirst({ where: { userId: user.id, token: firstToken } });
    if (!staleRow || staleRow.usedAt === null) throw new Error('staleRow not invalidated');

    const oldAttemptRes = makeMockRes();
    await handleVerifyFn({ method: 'POST', body: { email, code: firstCode } } as any, oldAttemptRes as any);
    if (oldAttemptRes._get().statusCode !== 401) throw new Error('old code accepted');

    const latestAttemptRes = makeMockRes();
    await handleVerifyFn({ method: 'POST', body: { email, code: latestCode } } as any, latestAttemptRes as any);
    if (latestAttemptRes._get().statusCode !== 200) throw new Error('latest code rejected');

    const replayRes = makeMockRes();
    await handleVerifyFn({ method: 'POST', body: { email, code: latestCode } } as any, replayRes as any);
...
async function main() {
  ...
  if (t === 'forgot-password') await runForgotPassword();
    ...
    console.log('Specify TEST env: forgot-password | login-rate | signup-rate | otp-resend');
  }
}
