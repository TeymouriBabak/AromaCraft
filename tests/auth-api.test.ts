...
test('check-username returns taken for mock user', async () => {
  ...
  assert.equal(
    ((out.body as Record<string, unknown>).data as Record<string, unknown>)
      .available,
    false
  );
});
...
test('check-email returns taken for mock email', async () => {
  const req = {
    method: 'GET',
    query: { email: 'tbabak@example.com' },
  } as unknown;
  const res = makeMockRes();
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
test('production does not silently select a mock SMS provider', () => {
  ...
  try {
    ...
    assert.throws(
      () => getSmsProvider(),
      /SMS_PROVIDER must be configured in production/
    );
  } finally {
...
if (!ONLY_TEST || ONLY_TEST === 'login-rate')
  test('login rate limiting returns 429 after repeated attempts and does not leak the account identifier', async () => {
      ...
      await (handleLogin as any)(
        {
          ...
          body: {
            ...
            password:
              process.env.TEST_CUSTOMER_PASSWORD || 'Test-Password-123!',
            role: 'customer',
          },
        },
        res
    ...
    assert.equal(
      (lastBody?.error as Record<string, unknown> | undefined)?.code,
      'rate_limited'
    );
    const errorMessage = String(
      (lastBody?.error as Record<string, unknown> | undefined)?.message ?? ''
    );
