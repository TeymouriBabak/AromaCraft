...
test('mobile normalization and uniqueness for Iranian numbers', async () => {
  ...
  assert.ok(
    canonical && canonical.startsWith('+98'),
    'Canonical should be +98 variant'
  );

  // find by alternative variant should resolve to the same user
  const found = await findUserByMobile(mobileVariants[1]);
  assert.ok(
    found && found.id === actualUser.id,
    'findUserByMobile should locate the created user from another variant'
  );
  ...
  assert.equal(
    user2,
    null,
    'Second user creation should return null due to mobile uniqueness conflict'
  );
});
