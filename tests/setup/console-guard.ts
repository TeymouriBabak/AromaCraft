...
const FALLBACK_PATTERNS = [
  /\[redis\].*continuing without Redis/i,
  /redis unavailable/i,
  /connection timeout/i,
  /rate-limit\] Redis unavailable/i,
];
...
beforeAll(() => {
  console.warn = (...args: unknown[]) => {
    ...
    return (origWarn as (...a: unknown[]) => void)(
  ...
  console.error = (...args: unknown[]) => {
    ...
    return (origError as (...a: unknown[]) => void)(
      ...(args as unknown as unknown[])
    );
  };
});
...
afterEach(() => {
  ...
  const hits = captured.filter((msg) =>
    FALLBACK_PATTERNS.some((rx) => rx.test(msg))
  );
  if (!allow && hits.length > 0) {
    ...
    throw new Error(
      `Unexpected Redis fallback logs detected: ${hits.join(' | ')}`
    );
  }
});
